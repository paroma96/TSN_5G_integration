...
class TSN_NETWORK:
    ...
    def __init__(self: object, path: list = [], reduceSpace: int = 4, pathLen: int = 4, \
        ...
        self.fluctuations: list = self.readFluct(path)
        ...
        self.radioDelay: list = self.readRadioDelay(path)
        if(self.N != len(self.radioDelay)  or self.N != len(self.radioDelay[0])):
            print("ERROR: RADIO DELAY VECTOR MISMATCHES DIMENSIONS.")
            return
        #--------------------------------------------------------------------------------------
    ...
    # set to 0.
    def readRadioDelay(self: object, path: list) -> list:
        with open(path + '/Inputs/radioDelMatrix.txt', 'r') as readM:
            next(readM)
    ...
    def sortFlows(self: object, flows: list) -> list:
        #flows=sorted(flows, key = lambda x: (x[3], x[4], x[2], x[0], x[1]))
                ...
                if(not initialized): 
                    ...
                    for ph in range(len(t0)):
                        if(network.flows[flowID].schEnd[i-1][ph] != 0):
                            ...
                            network.radioDelay[path[i]][path[i+1]]) # Prev. node end time + Bridge delay
                       ...
                       (network.flows[flowID].T_tx / network.order) == 0 and \
                        ...
                        if(port.schUP[phase+j] < t0_gb[phase+j]): 
                            ...
                            if(tpp <= port.schLW[phase+j] and leftSide and initialized):
                                ...
                                if(t0_gb[phase+j] < port.schUP[phase+j] and initialized): # Check path
                                    ...
                                    for ph in range(len(t0)):
                                        if(network.flows[flowID].schEnd[i-1][ph] != 0):
                                            t0[ph] = network.flows[flowID].schEnd[i-1][ph] + \
                                            network.nodes[path[i]].procDelay # Prev. node end time + proc. delay
                                            t0[ph] = round((t0[ph] % network.minPeriod), network.precision)
                                    goto .begin # In case t0_gb is slowed down so now t1=t0_gb > schUP
                                    #t0_gb = [round((ph + guardBand_p), network.precision) for ph in t0]
                                #---------------------------------------------------------------
...
def fitness_func(sol_chr: list, chr_pop_idx: int) -> float:
    ...
    #------------------------------------------------------------------------------------------
            ...
            # throughput plus the first and last nodes' radioaccess delay (if there is a radio channel it will
            # be more than cero, if Ethernet it is 0), respectively. Also, first node's and last node's
            # guard bands must be considered as well as their radio access delay. Flow's max. delay normalization
            node_init: int = network.flows[numFlow].pathNodes[0]
            hop_init: int = network.flows[numFlow].pathNodes[1]
            node_end: int = network.flows[numFlow].pathNodes[-1]
            ...
            delay_flow = (abs(delay1 - delay2) + \
                ...
                (1 + port_init.perc_gb) + network.radioDelay[node_init][hop_init] + \
                network.radioDelay[node_init][node_init] + network.radioDelay[node_end][node_end] + \
    ...
    #-----------------------------------------------------------------------------------------
"""-----------------------------------------------------------------------------------------"""
# Prints every evolutionary algorithm's generation.
...
# Debugging case
#path: str = "c:/Users/PaRoM/Desktop/TFM/AG" 
#flowState: str = "flowsVector_10"
#----------------------------------------------------------------------------------------------
if(len(sys.argv) >= 3):
...
#----------------------------------------------------------------------------------------------
# Checking
plot_top: bool = 0 # Plots the TSN topology (1 ON, 0 OFF)
...
if(1):
    ga_instance: object = pygad.GA(num_generations = num_generations,
                                   ...
                                   #on_generation = on_generation, # Shows generations' solutions
