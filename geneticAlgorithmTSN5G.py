                ...
                if(len(port_.gap[ph]) > 0): # Gaps observed
                    ...
                    for gap in port_.gap[ph]:
                        ...
                        if(gap[1] == port_.schUP[ph]):
                            del port_.gap[ph][-1]
                            if(len(port_.gap[ph]) == 0 and not gapped):
                                cnt_ph -= 1
                                break
                        else:
...
#----------------------------------------------------------------------------------------------
# Network parameters
mode5G: bool = 1 # 1 if 5G+TSN (5G files are read), 0 if TSN only <ATTENTION: SELECT TOPOLOGY>
...
boost: bool = 0 # Shortest path solution in initial population (used for test)
...
mutation_probability: float = float(sys.argv[5]) # Probability of gene mutation 
